...
{
    [HarmonyPatch(typeof(CargoPlaneAI), nameof(CargoPlaneAI.SimulationStep),
        new[] { typeof(ushort), typeof(Vehicle), typeof(Vector3) },
        new[] { ArgumentType.Normal, ArgumentType.Ref, ArgumentType.Normal })]
    internal static class Patch_CargoPlaneAI_SimulationStep
    {
        [HarmonyTranspiler]
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, MethodBase original) =>
            RandomizerRangeTranspiler.WidenGiveUpRoll(instructions, original, AccessTools.Method(typeof(Patch_CargoPlaneAI_SimulationStep), nameof(GetMultipliedRange)));
