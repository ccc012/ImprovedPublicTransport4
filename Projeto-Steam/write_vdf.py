...
CHANGENOTE = [
 u"Version 4.7.0 - four new integrations, reorganized Options menu",
 u"",
 u"FOUR NEW INTEGRATIONS",
 u"- Sub-Buildings Tabs: a tab bar on any multi-building hub's info panel, not just airports.",
 u"- Commuter Destination: adds a button to the stop panel showing where the passengers waiting there are actually headed.",
 u"- Shared Stop Enabler (off by default): lets more than one stop type share the same road segment.",
 u"- Taxi Stand Fix: idle taxis head for the nearest stand instead of wandering. An original IPT4 implementation, not a port.",
 u"- Every new integration has its own switch under Options - Integrations.",
 u"",
 u"GARAGE AND INTERCITY TERMINAL CAPACITY",
 u"- Added a capacity mode for intercity bus terminals: Disabled (this mod's existing behaviour, unchanged for current saves), Intermediate (a moderate fixed cap), or Realistic (the terminal's own vanilla capacity).",
 u"- Fixed the accept intercity buses checkbox on those terminals: it was wired to the same game property as the train shutdown state, so toggling it on a bus terminal could quietly put the whole building into the shutdown/evacuate state instead of just changing bus acceptance. It now has its own, correctly isolated per-terminal setting.",
 u"",
 u"TRAIN DISPLAY",
 u"- Added an Original colour theme that matches the layout of the source Train Display mod: header strip with speed and status, plus a route strip in the vehicle's actual line colour.",
 u"- Fixed the Simple, Dark and Light themes showing the wrong colours - a Unity rendering quirk was tinting against a texture that was never pure white. They now render the colour you actually pick.",
 u"",
 u"OPTIONS MENU REORGANIZED",
 u"- The old Auto Line tab had grown to hold seven unrelated settings groups. Split into Fleet and Scheduling, Budget and Prices, Line Colors, and a single Integrations tab that holds every absorbed mod's switch - ready for whatever gets added next.",
 u"- Restored the Shortcuts tab removed in an earlier version.",
 u"- Added a link to the project's GitHub page under Options - Advanced.",
 u"",
 u"OTHER FIXES",
 u"- The Ticket Prices tab was always falling back to its plain slider row instead of the game's own budget-style slider, due to a typo in a field lookup.",
 u"- Reduced repeated UI lookups in two panels that were re-searching the interface every single frame instead of once.",
 u"",
 u"CREDITS",
