...
namespace AlgernonCommons.UI
{
    ...
    public static class UITextures
    {
        ...
        public static UITextureAtlas GetTextureAtlas(string atlasName)
        {
            ...
            if (TextureCache.TryGetValue(atlasName, out UITextureAtlas cachedAtlas))
            {
                return cachedAtlas;
            }
            ...
            if (atlases == null)
            {
                Logging.Error("GetTextureAtlas: FindObjectsOfTypeAll returned null for UITextureAtlas");
                return null;
            }

            for (int i = 0; i < atlases.Length; ++i)
            {
                if (atlases[i] == null || atlases[i].name == null || !atlases[i].name.Equals(atlasName))
                {
                    continue;
                }

                // Found a matching name - if the number of sprites of this atlas is greater than the last one found, use this.
                int spriteCount = atlases[i].spriteNames?.Length ?? 0;
                if (spriteCount > selectedAtlasSpriteCount)
                {
                    selectedAtlas = i;
                    selectedAtlasSpriteCount = spriteCount;
                }
            }
