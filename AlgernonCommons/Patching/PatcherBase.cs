    ...
    public class PatcherBase
    {
        ...
        public virtual void PatchAll()
        {
            ...
            if (!Patched)
            {
                ...
                if (HarmonyHelper.IsHarmonyInstalled)
                {
                    ...
                    // WARNING: PatchAll() with no filter scans the *entire* executing assembly.
                    // In IPT4 that assembly also contains every Integration/* [HarmonyPatch]. Prefer
                    // ImprovedPublicTransport.Util.HarmonyScope.PatchNamespace for integration code,
                    // or override OnPatchAll / this method for selective targets.
                    if (string.IsNullOrEmpty(_harmonyID))
                    {
                        Logging.Error("PatcherBase.PatchAll called with null/empty HarmonyID - aborting");
                        return;
                    }
        ...
        public void ListMethods()
        {
            ...
            foreach (MethodBase patchedMethod in patchedMethods)
            {
                ...
                if (patches == null)
                {
                    logMessage.AppendLine("    (no patch info)");
                    continue;
                }
                ...
                // Was wrongly iterating patches.Prefixes three times (copy-paste), so Postfixes and
                // Transpilers never appeared in the diagnostic listing.
                foreach (Patch postfix in patches.Postfixes)
                {
                    logMessage.Append("        Postfix ");
